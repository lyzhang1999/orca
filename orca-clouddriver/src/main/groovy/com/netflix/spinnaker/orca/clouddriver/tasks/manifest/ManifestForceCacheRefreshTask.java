...
@Component
@Slf4j
public class ManifestForceCacheRefreshTask extends AbstractCloudProviderAwareTask
    implements Task, ManifestAware, RetryableTask {
  ...
  public TaskResult execute(@Nonnull Stage stage) {
    ...
    if (allManifestsProcessed(stageData)) {
      registry
          .timer(durationTimerId.withTags("success", "true", "outcome", "complete"))
          .record(duration, TimeUnit.MILLISECONDS);
  ...
  /**
   * Checks on the status of any pending on-demand cache refreshes. If a pending refresh has been
   * processed, adds the corresponding manifest to processedManifests; if a pending refresh is not
   * found in clouddriver or is invalid, removes the corresponding manifest from refreshedManifests
   */
  private void checkPendingRefreshes(String provider, StageData stageData, long startTime) {
    ...
    List<ScopedManifest> manifestsToCheck =
        refreshedManifests.stream()
  ...
  private RefreshStatus getRefreshStatus(PendingRefresh pendingRefresh, long startTime) {
    ...
    if (pendingRefresh.cacheTime == null
        || pendingRefresh.processedTime == null
        || scopedManifest == null) {
      ...
      log.info(
          "Pending refresh of {} was cached as a part of this request, but not processed",
          pendingRefresh);
  ...
  /**
   * Requests an on-demand cache refresh for any manifest without a refresh requests that is either
   * pending or processed. Adds each manifest to refreshedManifests; if the request to clouddriver
   * was immediately processed, also adds the manifest to processedManifests.
   */
  ...
  @Data
  private static class PendingRefresh {
